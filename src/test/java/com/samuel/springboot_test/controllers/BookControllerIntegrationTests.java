...
public class BookControllerIntegrationTests {
    ...
    public void testThatCreateBookReturnsHttpStatus201Created() throws Exception {
        ...
        BookDto testBookA = TestDataUtil.createTestBookDtoA(testAuthorA);

        String bookJson = objectMapper.writeValueAsString(testBookA);
        String url = "/books/" + testBookA.getIsbn();
        mockMvc.perform(
                MockMvcRequestBuilders.put(url).contentType(MediaType.APPLICATION_JSON).content(bookJson)
        ).andExpect(
    ...
    public void testThatCreateBookSuccessfullyReturnsSavedBook() throws Exception {
        ...
        mockMvc.perform(
                ...
                MockMvcResultMatchers.jsonPath("$.author.name").value(testAuthorA.getName())
        );
    }
